...
struct Small_allocator {
	...
	inline void* allocate() {
		...
		if(queue_head != queue_tail) {
			...
			if(((ull)queue_head & (queue_segment_size-1)) + 4*sizeof(void*) <= queue_segment_size)
				__builtin_prefetch(queue_head[3], 0, 0);// this will not cause RE
...
struct Allocator_pair{
	Allocator x;
	char unusedx[remaining(sizeof(Allocator))];
	Allocator y;
	char unusedy[remaining(sizeof(Allocator))];
	uint id_inuse;
	Allocator *a[2];
