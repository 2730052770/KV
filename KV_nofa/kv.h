...
struct KVS{
	...
	void tree_insert(Node *node, uint tree_tag, void *old_son, void *new_son, uint fa_level, uint id, uint batch);
		...
		*/
		...
		for(uint id = batch-1; id < batch; id --) {// note id is unsigned
			...
			if(qy->req_type == REQ_PUT && qy->resp_type == RESP_EMPTY) {
				uint old_key = *(uint*)qy->old_block->kv.content;
				Node *father = route[global_level-1][id];
				...
				if(old_key == qy->tree_tag) {
					...
					tree_insert(father, qy->tree_tag, qy->old_block, qy->new_block, global_level-1, id, batch);
