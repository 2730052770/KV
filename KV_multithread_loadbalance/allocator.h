...
struct Small_allocator {
	...
	inline void* allocate() {
		...
		void * ret;
		if(free_block_head) {
			ret = free_block_head + 8;
			free_block_head = (char *) ((Free_block*)free_block_head) -> next;
		}
		else { 
			ret = uninitialized_block_head + 8;
			uninitialized_block_head += block_size + 8;
			if(uninitialized_block_head - segment_head + block_size + 8 > segment_size) allocate_segment();
		}
...
struct Allocator{
	...
	inline void free(void *ptr) {
		uc *buf = (uc*)ptr - 1;
		if(unlikely(*buf == n_category)) {
			printf("huge allocator cannot be used in multithread situation\n");
			exit(1);
			ha.free(buf);
		}
